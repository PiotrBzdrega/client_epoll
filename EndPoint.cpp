...
namespace COM
{
    EndPoint::EndPoint(std::string_view &ip, std::string_view &port, bool ssl) : _ip{ip}, _port{port}
    {
...
/* modify the behavior of the socket */
    ...
    int EndPoint::write(const void* buffer, int count)
    {
        SSL *ssl_obj = _tls();
